...
app.post('/render', async (req, res) => {
  ...
  const compositionId = 'Output';

  // You only have to do this once, you can reuse the bundle.
  const entry = path.resolve(__dirname, '../../packages/remotion/index.ts');
  // const entry = ""
  ...
  const bundleLocation = await bundle(path.resolve(entry), () => undefined, {
    ...
    webpackOverride: (config) => {
      ...
      // console.log("config", json.module.rules)
      // return {
      //   ...config,
      //   ...enableMdx
      // }
      return enableMdx(config)
    }
  });
 ...
 Review "${entry}" for the correct ID.`);
  ...
  res.json({ location: `//localhost:4000/renders/${fileName}` });
});
